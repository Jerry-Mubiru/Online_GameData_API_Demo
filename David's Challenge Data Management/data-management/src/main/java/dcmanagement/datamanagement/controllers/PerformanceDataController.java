...
@RestController
@RequestMapping("/api/performanceData")
public class PerformanceDataController {
    //stores the bucket name from dependencies
    @Value("$gcp.bucket.performance.name")
    private String performanceBucketName;
    ...
    public ResponseEntity<InputStream> getData() throws IOException
    {
        ...
        Blob blob = storage.get(performanceBucketName, "performanceData.txt");
    ...
    public ResponseEntity<String> initializeData(String newContent)
    {
        ...
        if(blob == null)
        {
            BlobId identifiers = BlobId.of(performanceBucketName,"performanceData.txt");
            BlobInfo info = BlobInfo.newBuilder(identifiers).build();
            storage.create(info, newContent.getBytes());
            return ResponseEntity.ok("Performance data initialized successfully!");
        }
    ...
    public ResponseEntity<String> updateData(String newContent)
    {
        ...
        BlobInfo update = BlobInfo.newBuilder(performanceBucketName, newContent)
